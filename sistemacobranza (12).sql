...
-- Tiempo de generación: 16-11-2018 a las 20:35:22
...
SELECT al.idAlumno FROM alumno al INNER JOIN matricula ma ON ma.Alumno_idAlumno=al.idAlumno; 
...
SET @Nombre=(SELECT CONCAT(p.nombrePersona,' ',p.apellidoPaterno,' ',p.apellidoMaterno) FROM alumno al INNER JOIN persona p ON p.idPersona=al.Persona_idPersona WHERE al.idAlumno=v_idAs);

SET @CuotaTotales=(SELECT COUNT(*) as NumeroCuota FROM cuota cu WHERE  cu.fechaEmision<=diaTemporal and cu.Alumno_idAlumno=v_idAs);

SET @CuotaPendiente=(SELECT COUNT(*) as NumeroCuota FROM cuota cu WHERE cu.fechaEmision<=diaTemporal and (cu.Estado_idEstado=5 OR cu.Estado_idEstado=6) and cu.Alumno_idAlumno=v_idAs);

SET @CuotaPagada=(SELECT COUNT(*) as NumeroCuota FROM cuota cu WHERE cu.fechaEmision<=diaTemporal and cu.Estado_idEstado=7 and cu.Alumno_idAlumno=v_idAs);

SET @CuotaVencida=(SELECT COUNT(*) as NumeroCuota FROM cuota cu WHERE  cu.fechaEmision<=diaTemporal and (cu.Estado_idEstado=5 OR cu.Estado_idEstado=6) and (DATEDIFF(DATE_FORMAT(NOW(),"%Y-%m-%d"),cu.fechaVencimiento)>0) and cu.Alumno_idAlumno=v_idAs);  
  
 
INSERT INTO TablaTemporal(fecha,CuotaTotal,CuotaPendiente,CuotaPagada,CuotaVencida,AlumnoNombre) VALUES (diaTemporal,@CuotaTotales,@CuotaPendiente,@CuotaPagada,@CuotaVencida,@Nombre); 
...
(1047, 4, NULL, '200.00', '100.00', '0.00', 2018, 11, '2018-11-01', '2018-11-30', 6),
(1048, 4, NULL, '200.00', '200.00', '0.00', 2018, 12, '2018-12-01', '2018-12-30', 5);
...
(1, 1, 'admin', '$2a$08$RCuzW/8g2Lg4QMNCfmsa/uKp33rvDmdWrC.P40DOECJlMtPu16NMW', 'Administrador', '2018-09-29 14:03:44', '::1', '2018-11-16 00:19:51');
